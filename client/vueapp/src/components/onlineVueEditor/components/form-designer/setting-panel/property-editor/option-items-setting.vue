<template>
  <div class="option-items-pane">
    ...
    >
      <draggable tag="ul" :list="optionModel.optionItems" v-bind="{ group: 'optionsGroup', ghostClass: 'ghost', handle: '.drag-option' }">
    ...
    <gd-selectTree
      v-else-if="selectedWidget.type === 'select-tree'"
      :cur-value="optionModel.defaultValue"
      :data="optionModel.optionItems"
      :obj="optionModel.props"
      :placeholder="i18nt('render.hint.selectPlaceholder')"
      @getValue="emitDefaultValueChange"
    />
    <el-cascader
    ...
    <div>
      <template v-if="['radio', 'checkbox', 'select'].includes(selectedWidget.type)">
      ...
      <template v-if="['select-tree', 'cascader'].includes(selectedWidget.type)">
    ...
    <gd-dialog
      ...
      :closed.sync="showImportDialogFlag"
    ...
    <gd-dialog
      :title="i18nt('designer.setting.importOptions')"
      :closed.sync="showImportCascaderDialogFlag"
      class="small-padding-dialog"
      :maximized.sync="maximized"
      draggable
      maximizable
  ...
  methods: {
    emitDefaultValueChange() {
      if (!!this.designer && !!this.designer.formWidget) {
        const fieldWidget = this.designer.formWidget.getWidgetRef(this.selectedWidget.options.name)
