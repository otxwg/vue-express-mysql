<template>
  <div class="container-wrapper" :class="[customClass]" :style="styleObj">
    <slot></slot>
    ...
    <div class="container-action" v-if="designer.selectedId === widget.id && !widget.internal">
      ...
      <i v-if="widget.type === 'table'" class="iconfont icon-insertrow" :title="i18nt('designer.hint.insertRow')" @click.stop="insertTableRow(widget)"></i>
      <i v-if="widget.type === 'table'" class="iconfont icon-insertcolumn" :title="i18nt('designer.hint.insertColumn')" @click.stop="insertTableCol(widget)"></i>
      <i
