<template>
  <div class="field-wrapper" :class="{ 'design-time-bottom-margin': !!this.designer }" :style="styleObj">
    <el-form-item
      ...
      <span v-if="!!field.options.labelIconClass" slot="label" class="custom-label">
        <template v-if="field.options.labelIconPosition === 'front'">
          <template v-if="!!field.options.labelTooltip">
            <el-tooltip :content="field.options.labelTooltip" effect="light"> <i :class="field.options.labelIconClass"></i></el-tooltip>{{ label }}</template
          >
        ...
        <template v-else-if="field.options.labelIconPosition === 'rear'">
          <template v-if="!!field.options.labelTooltip">
            {{ label }}<el-tooltip :content="field.options.labelTooltip" effect="light"> <i :class="field.options.labelIconClass"></i></el-tooltip
          ></template>
    ...
    <template v-if="!!this.designer">
      <div class="field-action" v-if="designer.selectedId === field.id">
        ...
        <i class="el-icon-top" v-if="!!parentList && parentList.length > 1" :title="i18nt('designer.hint.moveUpWidget')" @click.stop="moveUpWidget(field)"></i>
        <i class="el-icon-bottom" v-if="!!parentList && parentList.length > 1" :title="i18nt('designer.hint.moveDownWidget')" @click.stop="moveDownWidget(field)"></i>
...
export default {
  ...
  computed: {
    ...
    labelAlign() {
      ...
      if (this.designer) {
        return this.designer.formConfig.labelAlign
      } else {
        return this.formConfig.labelAlign
      }
    },
